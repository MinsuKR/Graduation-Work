...
bool waitForAny(const char* key1, const char* key2, const char* key3, uint32_t timeoutMs)
{
  ...
  while (millis() - start < timeoutMs) {
    while (WIFI_SERIAL.available()) {
      ...
      String ipdData = extractIpdPayload();
      if (ipdData.length() > 0) {
        pendingReply = ipdData;
      }
...
String extractIpdPayload()
{
  ...
  if (len > 0 && availableLen >= len) {
    ...
    data.trim();
    rxBuf = rxBuf.substring(dataStart + len);
    return data;
  }
  ...
  if (newline >= 0) {
    ...
    data.trim();
    rxBuf = rxBuf.substring(newline + 1);
    return data;
  }
...
bool tcpSendLine(const String& data)
{
  ...
  // Do not flush here. A delayed +IPD reply may already be waiting in UART.
  sendAt(cmd);
  if (!waitForAny(">", NULL, NULL, 3000)) return false;

  WIFI_SERIAL.print(payload);
  return waitForAny("SEND OK", NULL, NULL, 5000);
}
